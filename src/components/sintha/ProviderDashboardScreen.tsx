...
import {
  ...
  ShieldCheck, ChevronRight, Play, AlertCircle, Sparkles, Loader2, Lightbulb, X
} from 'lucide-react'
import { useToast } from '@/hooks/use-toast'
import { cleanError } from '@/lib/clean-error'

// Shape returned by POST /api/ai/optimize-profile
interface AiProfileSuggestion {
  score: number
  strengths: string[]
  improvements: Array<{ area: string; suggestion: string; priority: 'high' | 'medium' | 'low' }>
  suggestedDescription: string
  suggestedRate?: { current?: number | null; suggested?: number | null; reason?: string }
  tips: string[]
  poweredBy?: string
  error?: string
}
...
export default function ProviderDashboardScreen() {
  ...
  // aiOptimizing = loading state for the "Optimize My Profile" call.
  // aiSuggestions = the structured suggestions returned by SINTHA AI.
  const [aiOptimizing, setAiOptimizing] = useState(false)
  const [aiSuggestions, setAiSuggestions] = useState<AiProfileSuggestion | null>(null)
  ...
  const optimizeProfile = async () => {
    ...
    try {
      const data = await apiFetch<AiProfileSuggestion>('/ai/optimize-profile', {
        method: 'POST',
        body: JSON.stringify({ providerId: user.id }),
      })
      setAiSuggestions(data)
      if (data.error) {
        toast({ title: 'AI unavailable', description: data.error })
      }
    } catch (err) {
      toast({ title: 'Failed to analyze profile', description: cleanError(err) })
    } finally {
      setAiOptimizing(false)
    }
  }

  const closeAiSuggestions = () => setAiSuggestions(null)
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="px-4 py-4 space-y-4">
        ...
        {/* AI Profile Optimizer — purple gradient card.
            Calls POST /api/ai/optimize-profile with the provider's userId,
            then surfaces a score, strengths, prioritized improvements,
            a rewritten description, and quick tips. */}
        <div className="bg-gradient-to-br from-purple-600 to-violet-700 rounded-2xl p-4 shadow-sm text-white relative overflow-hidden">
          <div className="absolute -right-6 -top-6 w-24 h-24 bg-white/10 rounded-full" />
          <div className="absolute right-10 -bottom-4 w-16 h-16 bg-white/5 rounded-full" />
          <div className="relative z-10">
            <div className="flex items-center gap-2 mb-2">
              ...
              <div className="flex-1">
                <p className="text-sm font-bold leading-tight">AI Profile Optimizer</p>
                <p className="text-[10px] text-white/70 leading-tight">
                  Get a profile score &amp; suggestions to win more bookings
                </p>
              </div>
              <span className="text-[9px] font-bold bg-white/20 px-2 py-0.5 rounded-full tracking-wider">
                SINTHA AI
              </span>
            </div>
            ...
            <button
              ...
              disabled={aiOptimizing || !user}
              className="w-full bg-white text-purple-700 rounded-xl py-2.5 text-sm font-bold flex items-center justify-center gap-2 disabled:opacity-60 active:scale-[0.99] transition-transform"
            >
              {aiOptimizing ? (
                <>
            ...
            {aiOptimizing && (
              <div className="mt-3 space-y-2">
                <div className="bg-white/15 rounded-xl h-20 animate-pulse" />
                <div className="bg-white/15 rounded-xl h-16 animate-pulse" />
              </div>
            )}

            {/* Suggestions */}
            {!aiOptimizing && aiSuggestions && (
              <div className="mt-3 space-y-3 relative">
                {/* Close button */}
                <button
                  onClick={closeAiSuggestions}
                  className="absolute -top-1 right-0 w-6 h-6 rounded-full bg-white/20 flex items-center justify-center hover:bg-white/30 z-10"
                ...
                <div className="bg-white/15 rounded-xl p-3 flex items-center gap-3">
                  <div className="relative w-14 h-14 shrink-0">
                    <svg className="w-14 h-14 -rotate-90" viewBox="0 0 36 36">
                      ...
                      <circle
                        cx="18" cy="18" r="15" fill="none" stroke="#ffffff" strokeWidth="3"
                        strokeDasharray={`${((aiSuggestions.score || 0) / 100) * 94.2} 94.2`}
                        strokeLinecap="round"
                      />
                    </svg>
                    <span className="absolute inset-0 flex items-center justify-center text-sm font-bold text-white">
                      {Math.round(aiSuggestions.score || 0)}
                    </span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-bold">Profile Score</p>
                    <p className="text-[10px] text-white/80">
                      {(aiSuggestions.score || 0) >= 90 ? 'Excellent — top tier!' :
                       (aiSuggestions.score || 0) >= 70 ? 'Good — minor tweaks will help' :
                       (aiSuggestions.score || 0) >= 50 ? 'Average — room to grow' :
                ...
                {aiSuggestions.strengths && aiSuggestions.strengths.length > 0 && (
                  <div className="bg-white/15 rounded-xl p-3">
                    <p className="text-[10px] font-bold uppercase tracking-wide text-white/80 mb-2">
                      Strengths
                    </p>
                    <ul className="space-y-1.5">
                      {aiSuggestions.strengths.map((s, i) => (
                        <li key={i} className="flex items-start gap-2 text-xs">
                          <CheckCircle className="h-3.5 w-3.5 shrink-0 mt-0.5 text-green-300" />
                          <span>{s}</span>
                        </li>
                      ))}
                    </ul>
                ...
                {/* Improvements with priority badges */}
                    ...
                    <ul className="space-y-2">
                      {aiSuggestions.improvements.map((imp, i) => (
                        <li key={i} className="text-xs">
                          ...
                          <p className="text-white/85 leading-snug">{imp.suggestion}</p>
                        </li>
                      ))}
                    </ul>
                  </div>
                ...
                {aiSuggestions.suggestedDescription && (
                  <div className="bg-white/15 rounded-xl p-3">
                    ...
                    <p className="text-xs leading-relaxed text-white/95 italic">
                      &ldquo;{aiSuggestions.suggestedDescription}&rdquo;
                    </p>
                    <button
                      onClick={() => navigate('provider-onboarding')}
                      className="mt-2 text-[10px] font-bold bg-white text-purple-700 px-2 py-1 rounded-md"
                    >
                      Edit Profile →
                    </button>
                  </div>
                ...
                {/* Suggested rate */}
                {aiSuggestions.suggestedRate &&
                  aiSuggestions.suggestedRate.suggested != null &&
                  aiSuggestions.suggestedRate.suggested !== aiSuggestions.suggestedRate.current && (
                  <div className="bg-white/15 rounded-xl p-3">
                    <p className="text-[10px] font-bold uppercase tracking-wide text-white/80 mb-1">
                      Rate Suggestion
                    </p>
                    <p className="text-xs">
                      {aiSuggestions.suggestedRate.current != null && (
                        <>
                          <span className="line-through text-white/60">
                            ₹{aiSuggestions.suggestedRate.current}/hr
                          </span>
                          {' → '}
                        </>
                      )}
                      <span className="font-bold">₹{aiSuggestions.suggestedRate.suggested}/hr</span>
                    </p>
                    {aiSuggestions.suggestedRate.reason && (
                      <p className="text-[10px] text-white/80 mt-1">{aiSuggestions.suggestedRate.reason}</p>
                    )}
                  </div>
                )}
                ...
                {aiSuggestions.tips && aiSuggestions.tips.length > 0 && (
                  <div className="bg-amber-300/20 rounded-xl p-3">
